  ...
  <table id="rounded-corner">
    <thead>
      <tr>
        ...
        <th scope="col" class="rounded">Editorial</th>
        <th scope="col" class="rounded">Descripci&oacute;n</th>
    ...
    <tfoot>
      <tr>
        <td colspan="4" class="rounded-foot-left"><em>Mantemiento de Editoriales del Sistema, puede agregar, editar o eliminar alg&uacute;n registro.</em></td>
    ...
    {foreach item="data" from="$_result"}
      <tr>
        <td><input type='checkbox' name='id_user' value='{$data.publisher_id}' /></td>
        <td>{$data.publisher}</td>
        <td>{$data.description}</td>
        <td><a class='dataPublisher' href='javascript:void(0);' title='Editar a {$data.publisher}' p='{$data.publisher_id}'><img src='{$_layoutParams.ruta_img}user_edit.png' /></a></td>
        <td><a href='javascript:void(0);' class='ask' title='Eliminar a {$data.publisher}' onclick="deleteRow('{$data.publisher}', '{$data.publisher_id}')"><img src='{$_layoutParams.ruta_img}trash.png' /></a></td></tr>
    {/foreach}
    </tbody>
  </table>  
  <a id='addPublisher' href='javascript:void(0);' class='bt_green' title='Agregar editorial'><strong>Agregar editorial</strong></a>
